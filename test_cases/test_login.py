...
test_data_with_empty_data = get_CSV_data_as_dict(filename)


@pytest.mark.log_in
@pytest.mark.parametrize("login, password", test_data_with_empty_data)
def test_login_with_empty_credentials(driver, login, password):
    """
    Preconditions:
    The application is enabled on the start page - login page.
    1.Insert login credentials
    ...
    login_page = UserInteractions(driver)
    login_page.login(login, password)
    logging_with_empty_data = MainPage(driver)

    assert logging_with_empty_data.get_window_header_text_empty_login == "Username is required", 'Header is not displayed'
